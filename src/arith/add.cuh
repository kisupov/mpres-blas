 ...
 * result = x + y
 */
GCC_FORCEINLINE void mp_add(mp_float_ptr result, mp_float_t x, mp_float_t y) {
    int ex = x.exp;
    int ey = y.exp;
    int sx = x.sign;
    int sy = y.sign;
    ...
    unsigned char nzx = ((y.eval[1].frac == 0) || (theta + y.eval[1].exp) < MP_J); //nzx (u) = 1 if x not need be zeroed; otherwise nzx = 0
    unsigned char nzy = ((x.eval[1].frac == 0) || (gamma + x.eval[1].exp) < MP_J); //nzy (v) = 1 if y not need be zeroed; otherwise nzy = 0
    ...
    x.eval[0].frac *=  factor_x;
    x.eval[1].frac *=  factor_x;
    y.eval[0].frac *=  factor_y;
    y.eval[1].frac *=  factor_y;

    //Interval addition
    result->eval[0] = er_add_rd(x.eval[sx], y.eval[sy]);
    result->eval[1] = er_add_ru(x.eval[1 - sx], y.eval[1 - sy]);
...
namespace cuda {
    ...
    DEVICE_CUDA_FORCEINLINE void mp_add_common(int * sr, int * er, er_float_ptr * evlr, int * digr,
                                               int sx, int ex, er_float_t * evalx, const int * digx,
                                               int sy, int ey, er_float_t * evaly, const int * digy)
    {
        ...
        *evlr[0] = cuda::er_add_rd(evalx[sx], evaly[sy]);
        *evlr[1] = cuda::er_add_ru(evalx[1 - sx], evaly[1 - sy]);

        //Sign identification
        unsigned char sign = evlr[0]->frac < 0; // 1 if negative, 0 if positive
        if(sign != (evlr[1]->frac < 0)){
     ...
     * result = x + y
     */
    DEVICE_CUDA_FORCEINLINE void mp_add(mp_float_ptr result, mp_float_t x, mp_float_t y) {
     ...
     */
    DEVICE_CUDA_FORCEINLINE void mp_add(mp_float_ptr result, mp_float_t x, mp_array_t y, int idy) {
        er_float_t evalx[2] = { x.eval[0], x.eval[1] };
     ...
     */
    DEVICE_CUDA_FORCEINLINE void mp_add(mp_array_t result, int idr, mp_array_t x, int idx, mp_float_t y) {
     ...
     * @param result - pointer to the computed sum, result = x + y[idy]
     */
    DEVICE_CUDA_FORCEINLINE void mp_add(mp_float_ptr result, mp_float_t x, mp_collection_t y, int idy, int leny) {
        er_float_t evalx[2] = { x.eval[0], x.eval[1] };
        er_float_t evaly[2] = { y.eval[idy], y.eval[idy + leny] };
        er_float_ptr evalr[2] = { &result->eval[0], &result->eval[1] };
        mp_add_common(&result->sign, &result->exp, evalr, result->digits,
                      x.sign, x.exp, evalx, x.digits,
                      y.sign[idy], y.exp[idy], evaly, &y.digits[RNS_MODULI_SIZE * idy]);
