    ...
    */
    __global__ void mp_matrix_add_esi_kernel(mp_array_t R, const int ldr, mp_array_t A, const int lda, mp_array_t B, const int ldb, const int m, const int n) {
        ...
        if (ix < m && iy < n){
                ...
                //Interval addition
                R.eval[idr] = cuda::er_add_rd(evalA[signA], evalB[signB]);
                R.eval[idr + lenR] = cuda::er_add_ru(evalA[1 - signA], evalB[1 - signB]);
    ...
    __global__ void mp_mat2scal_mul_esi_kernel(mp_array_t R, const int ldr, mp_array_t A, const int lda, mp_array_t alpha, const int m, const int n) {
        ...
        if (ix < m && iy < n){
            ...
            R.eval[idr] = cuda::er_md_rd(A.eval[ida], alpha_ev0, cuda::RNS_EVAL_UNIT.upp);
            R.eval[lenr + idr] = cuda::er_md_ru(A.eval[lena + ida], alpha_ev1, cuda::RNS_EVAL_UNIT.low);
        }
    }
    ...
    __global__ void mp_mat2vec_right_scal_esi_kernel(mp_array_t R, const int ldr, mp_array_t A, const int lda, mp_array_t x, const int incx, const int m, const int n) {
        ...
        while (colId < n){
            ...
            while (ide < m) {
                ...
                R.eval[idr + ide] = cuda::er_md_rd(A.eval[ida + ide],  x_ev0, cuda::RNS_EVAL_UNIT.upp);
                R.eval[lenr + idr + ide] = cuda::er_md_ru(A.eval[lena + ida + ide], x_ev1, cuda::RNS_EVAL_UNIT.low);
                //Go to the next iteration
    ...
    __global__ static void mp_mat2vec_left_scal_esi_kernel(mp_array_t R, int ldr, mp_array_t A, int lda, mp_array_t x, const int incx, const int m, const int n) {
        ...
        while (colId < n){
            ...
            while (numberIdx < m) {
                ...
                R.eval[idr + numberIdx] = cuda::er_md_rd(A.eval[ida + numberIdx], x.eval[ix], cuda::RNS_EVAL_UNIT.upp);
                R.eval[lenr + idr + numberIdx] = cuda::er_md_ru(A.eval[lena + ida + numberIdx], x.eval[lenx + ix], cuda::RNS_EVAL_UNIT.low);
