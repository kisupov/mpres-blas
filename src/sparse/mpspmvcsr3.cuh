 ...
 *  Third CSR implementation (vector kernel) - a half-warp (16 threads) is assigned to each row of the matrix
 *
...
namespace cuda {
     ...
     * Vector kernel - a half-warp (16 threads) is assigned to each row of the matrix, i.e. one element of the vector y.
     ...
     */
    __global__ void mpspmv_csr3(const int m, const int *irp, const int *ja, mp_float_ptr as, mp_float_ptr x, mp_float_ptr y) {
        extern __shared__ mp_float_t vals[];

        auto threadId = threadIdx.x + blockIdx.x * blockDim.x; // global thread index
        auto halfWarpId = threadId / 16; // global half-warp index
        auto lane = threadId & (16 - 1); // thread index within the half-warp
        auto row = halfWarpId; // one warp per row
        ...
        while (row < m) {
            ...
            for (int i = row_start + lane; i < row_end; i += 16) {
