/*
 *  Preconditioned multiple precision CG iterative method for solving linear systems using the DIA storage format
...
#ifndef MPRES_PCG_DIA_CUH
#define MPRES_PCG_DIA_CUH
...
/**
 * Preconditioned multiple precision conjugate gradient linear solver (Ax = b) using DIA storage for the matrix
 * ASSUMPTIONS:
 ...
 * @param A - sparse double-precision matrix in the DIA storage format
...
int mp_pcg_dia(const int n, const int ndiag, const jad_t &A, mp_float_ptr b, const double tol, const int maxit, enum preconditioner_type ptype, double *M, mp_float_ptr x, vector<double> &resvec){
    ...
    cuda::mp_spmv_dia<threads><<<blocks, threads>>>(n, n, ndiag, A, x, r);
    ...
    while(normk > epsilon && k < maxit){
        ...
        //q = Ap
        cuda::mp_spmv_dia<threads><<<blocks, threads>>>(n, n, ndiag, A, p, q);
