/*
 *  Preconditioned multiple precision CG iterative method for solving linear systems using the Jagged Diagonal storage format
...
#ifndef MPRES_PCG_JAD_CUH
#define MPRES_PCG_JAD_CUH
...
/**
 * Preconditioned multiple precision conjugate gradient linear solver (Ax = b) using JAD storage for the matrix
 * ASSUMPTIONS:
...
int mp_pcg_jad(const int n, const int maxnzr, const jad_t &A, mp_float_ptr b, const double tol, const int maxit, enum preconditioner_type ptype, double *M, mp_float_ptr x, vector<double> &resvec){
