/*
 *  Multiple precision CG iterative method for solving linear systems using the DIA matrix storage format
...
#ifndef MPRES_CG_DIA_CUH
#define MPRES_CG_DIA_CUH
...
/**
 * Multiple precision conjugate gradient linear solver (Ax = b) using DIA storage for the matrix
 * ASSUMPTIONS:
...
int mp_cg_dia(const int n, const int ndiag, const ell_t &A, mp_float_ptr b, const double tol, const int maxit, mp_float_ptr x, std::vector<double> &resvec){
