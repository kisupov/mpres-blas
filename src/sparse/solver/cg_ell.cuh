/*
 *  Multiple precision CG iterative method for solving linear systems using the ELLPACK matrix storage format
...
#ifndef MPRES_CG_ELL_CUH
#define MPRES_CG_ELL_CUH
...
/**
 * Multiple precision conjugate gradient linear solver (Ax = b) using ELLPACK storage for the matrix
 * ASSUMPTIONS:
...
int mp_cg_ell(const int n, const int maxnzr, const ell_t &A, mp_float_ptr b, const double tol, const int maxit, mp_float_ptr x, std::vector<double> &resvec){
