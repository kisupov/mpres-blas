/*
 *  Preconditioned (Jacobi preconditioner) multiple precision CG iterative method for solving linear systems using the CSR matrix storage format
 *
...
#ifndef MPRES_PCGCSRLS_JACOBI_H
#define MPRES_PCGCSRLS_JACOBI_H
 ...
 * Jacobi-preconditioned multiple precision conjugate gradient linear solver (Ax = b) using CSR for storing the matrix
 *
 ...
 */
int mp_pcgcsrls_jacobi(const int n, const csr_t &matrix, mp_float_ptr rhs, mp_float_ptr x, mp_float_ptr r, mp_float_ptr p, mp_float_ptr q, mp_float_ptr z, mp_float_ptr precond, const double tol, const double maxit){
    ...
    while(normk > epsilon && k < maxit){
        ...
        cuda::mp_prod<<<BLOCKS, THREADS>>>(n, precond, r, z);
        checkDeviceHasErrors(cudaMemcpy(rhop, rho, sizeof(mp_float_t), cudaMemcpyDeviceToDevice)); //rhop = rho
        cuda::mp_dot<BLOCKS_SCALAR,THREADS>(n, r, z, rho); //rho = r^{T} z
        if(k == 0){
            checkDeviceHasErrors(cudaMemcpy(p, z, sizeof(mp_float_t) * n, cudaMemcpyDeviceToDevice)); //p = z
        } else{
            cuda::mp_div(beta, rho, rhop); //beta = rho_{k-1} / rho_{k-2}
            cuda::mp_axpy<<<BLOCKS, THREADS>>>(n, beta, p, z, p); // pk = z_{k−1} + β_{k−1} * p_{k−1}
        }
