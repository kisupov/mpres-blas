...
namespace cuda {
     ...
     * The component wise products are written into the result array of size m * maxnz
     * @note The 'as' and 'result' arrays are treated as vectors (one-dimensional arrays)
     */
    __global__ void ell_scal_esi_kernel(const int m, const int n, const int maxnz, const int *ja, mp_collection_t as, mp_array_t x, mp_collection_t result) {
        ...
        while (numberIdx < len) {
            ...
            if(index >= 0) {
                ...
                cuda::er_md_ru(&result.eval[len + numberIdx], &as.eval[len + numberIdx], &x.eval[n + index], &cuda::RNS_EVAL_UNIT.low);
            }
     ...
     * @note The 'as' and 'result' arrays are treated as vectors (one-dimensional arrays)
     */
    __global__ void ell_scal_digits_kernel(const int m, const int maxnz, const int *ja, mp_collection_t as, mp_array_t x, mp_collection_t result){
        ...
        while (digitId < m * maxnz * RNS_MODULI_SIZE) {
     ...
     */
    __global__ static void ell_matrix_row_sum_kernel(const int m, const int maxnz, const int *ja, mp_collection_t as, mp_array_t y){
        extern __shared__ mp_float_t sum[];
        auto row = threadIdx.x + blockIdx.x * blockDim.x;
        if (row < m) {
            sum[threadIdx.x] = cuda::MP_ZERO;
            for (auto col = 0; col < maxnz; col++) {
                auto index = ja[col * m + row];
                if(index >= 0){
                    cuda::mp_add(&sum[threadIdx.x], &sum[threadIdx.x], as, col * m + row, m * maxnz);
                }
            }
     ...
     * @note Requires global memory buffer of size m * maxnz
     *
     ...
     * @param maxnz - number of nonzeros per row array (maximum number of nonzeros per row in the matrix A)
     * @param ja - column indices array to access the corresponding elements of the vector x, size = m * maxnz (the same as for A)
     * @param as - multiple-precision coefficients array (entries of the matrix A in the ELLPACK format), size = m * maxnz
     ...
     * @param buffer - auxiliary global memory array for storing the intermediate matrix, size = m * maxnz
     */
    template<int gridDim1, int blockDim1, int gridDim2, int blockDim3>
    void mpspmv_ell_2stage(const int m, const int n, const int maxnz, const int *ja, mp_collection_t &as, mp_array_t &x, mp_array_t &y, mp_collection_t &buffer) {

        //We consider the vector x as a diagonal matrix and perform the right diagonal scaling, buffer = A * x.
        //The result is written to the intermediate buffer of size m * maxnz.

        //Multiplication buffer = A * x - Computing the signs, exponents, and interval evaluations
        ell_scal_esi_kernel<<<gridDim1, blockDim1>>>(m, n, maxnz, ja, as, x, buffer);

        //Multiplication buffer = A * x - Multiplying the digits in the RNS
        ell_scal_digits_kernel<<< gridDim2, BLOCK_SIZE_FOR_RESIDUES >>> (m, maxnz, ja, as, x, buffer);

        //Rounding the intermediate result (buffer)
        mp_vector_round_kernel<<< 64, 64 >>> (buffer, m * maxnz);
        ...
        ell_matrix_row_sum_kernel<<<m / blockDim3 + 1, blockDim3, sizeof(mp_float_t) * blockDim3 >>>(m, maxnz, ja, buffer, y);
    }
