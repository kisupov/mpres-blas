...
namespace cuda {
     ...
     * The component wise products are written into the result array of size m * nzr
     * @note The 'as' and 'result' arrays are treated as vectors (one-dimensional arrays)
     */
    __global__ void ell_scal_esi_kernel(const int m, const int n, const int nzr, const int *ja, mp_collection_t as, mp_array_t x, mp_collection_t result) {
     ...
     * @note The 'as' and 'result' arrays are treated as vectors (one-dimensional arrays)
     */
    __global__ void ell_scal_digits_kernel(const int m, const int nzr, const int *ja, mp_collection_t as, mp_array_t x, mp_collection_t result){
        ...
        while (digitId < m * nzr * RNS_MODULI_SIZE) {
     ...
     */
    __global__ static void ell_matrix_row_sum_kernel(const int m, const int nzr, const int *ja, mp_collection_t as, mp_array_t y){
        extern __shared__ mp_float_t sum[];
        auto row = threadIdx.x + blockIdx.x * blockDim.x;
        if (row < m) {
            sum[threadIdx.x] = cuda::MP_ZERO;
            for (auto col = 0; col < nzr; col++) {
                auto index = ja[col * m + row];
                if(index >= 0){
                    cuda::mp_add(&sum[threadIdx.x], &sum[threadIdx.x], as, col * m + row, m * nzr);
                }
            }
     ...
     * @note Requires global memory buffer of size m * nzr
     *
    ...
    void mpspmv_ell2(const int m, const int n, const int nzr, const int *ja, mp_collection_t &as, mp_array_t &x, mp_array_t &y, mp_collection_t &buffer) {
        ...
        ell_scal_esi_kernel<<<gridDim1, blockDim1>>>(m, n, nzr, ja, as, x, buffer);

        //Multiplication buffer = A * x - Multiplying the digits in the RNS
        ell_scal_digits_kernel<<< gridDim2, BLOCK_SIZE_FOR_RESIDUES >>> (m, nzr, ja, as, x, buffer);

        //Rounding the intermediate result (buffer)
        mp_vector_round_kernel<<< 64, 64 >>> (buffer, m * nzr);
        ...
        ell_matrix_row_sum_kernel<<<m / blockDim3 + 1, blockDim3, sizeof(mp_float_t) * blockDim3 >>>(m, nzr, ja, buffer, y);
    }
