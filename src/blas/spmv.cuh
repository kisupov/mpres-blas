...
namespace cuda
{
     ...
     * y = A*x
     * where x and y are vectors and A is an sparse matrix.
    ...
    void spmv(enum mblas_trans_type trans, const int m, const int n, mp_array_t &A, const int lda,
            mp_array_t &x, const int incx, mp_array_t &y, const int incy, mp_array_t &buffer1, mp_array_t &buffer2){
        ...
        mp_mat2vec_right_scal_esi_kernel<<<grid1, blockDim1>>> (buffer2, m, A, lda, x, 1, m, n);

        //Multiplication buffer2 = A * buffer1 - Multiplying the digits in the RNS
        mp_mat2vec_right_scal_digits_kernel<<<grid2, BLOCK_SIZE_FOR_RESIDUES>>> (buffer2, m, A, lda, x, 1, m, n);
