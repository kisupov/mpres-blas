...
void gmpcssn( const double* a, const int interval_a, 
	...
	double f[6];
	new_gmp( sk0, interval_sk, 0.0, n5 );
	new_gmp( sk1, interval_sk, 0.0, n5 );
	new_gmp( sk2, interval_sk, 0.0, n5 );
	new_gmp( sk3, interval_sk, 0.0, n5 );
	new_gmp( sk4, interval_sk, 0.0, n5 );
	new_gmp( sk5, interval_sk, 0.0, n5 );
	new_gmp( sk6, interval_sk, 0.0, n5 );
	new_gmp( f, 1.0, 6 ) ;
	
	//mpmuld(pi, 2.0, 0, sk0, prec_words);
	gmpmuld(_pi, 1, 2.0, 0, sk0, interval_sk, prec_words, d, interval_d);
	//mpdiv(a, sk0, sk1, prec_words);
	gmpdiv(a, interval_a, sk0, interval_sk, sk1, interval_sk, prec_words, d, interval_d);
	//mpnint(sk1, sk2, prec_words);
	gmpnint(sk1, interval_sk, sk2, interval_sk, prec_words, d, interval_d);
	//mpsub(sk1, sk2, sk3, prec_words);
	gmpsub(sk1, interval_sk, sk2, interval_sk, sk3, interval_sk, prec_words, d, interval_d);


	

	//mpmdc(sk3, t1, n1, prec_words);
	gmpmdc(sk3, interval_sk, t1, n1, prec_words);
	if(n1 >= -MPNBT) {
		t1 = ldexp(t1, n1);
		t2 = 4.0 * t1;
		t1 = ganint (double(t2));
	...
	gmpdmc(t1, 0, sk1, interval_sk, prec_words);
	//mpsub(sk3, sk1, sk2, prec_words);
	gmpsub(sk3, interval_sk, sk1, interval_sk, sk2, interval_sk, prec_words, d, interval_d);
	//mpmul(sk2, sk0, sk1, prec_words);
	gmpmul(sk2, interval_sk, sk0, interval_sk, sk1, interval_sk, prec_words, d, interval_d);


	// compute consine and sine of the reduced argument s.

	if(mpr_get(sk1, interval_sk, 1) == 0.0) { // if sk1 == zero
		zero(sk0, interval_sk);
		nq = 0;
	} else {
		//Divide by 2^nq (possibly), fix after series has converged.
		//if(sk1.mpr[2] < -1 || sk1.mpr[3] < mpbdx/4096.0) {
		if(mpr_get(sk1, interval_sk, 2) < -1 || mpr_get(sk1, interval_sk, 3) < MPBDX/4096.0) {
			nq = 0;
		} else {
			//mpdivd(sk1, 1.0, nq, sk1, prec_words);
			gmpdivd(sk1, interval_sk, 1.0, nq, sk1, interval_sk, prec_words, d, interval_d);
		}

		// Compute the Taylor's series now.
		//mpeq(sk1, sk0, prec_words);
		gmpeq(sk1, interval_sk, sk0, interval_sk, prec_words);
		//mpmul(sk0, sk0, sk2, prec_words);
		gmpmul(sk0, interval_sk, sk0, interval_sk, sk2, interval_sk, prec_words, d, interval_d);

		int l1=0; //iteration count.
		int term_prec;

		neg = mpr_get(sk1, interval_sk, 1) < 0.0 ? 1 : 0;
		do {
			l1++;
			t2 = - (2.0 * l1) * (2.0 * l1 + 1.0);

			// compute this term with term_prec words of precision only.
			term_prec = min(nws+1, nws+int(mpr_get(sk2, interval_sk, 2) + 
						       mpr_get(sk1, interval_sk, 2) - 
						       mpr_get(sk0, interval_sk, 2))+2);
			...
			gmpdivd(sk3, interval_sk, t2, 0, sk1, interval_sk, prec_words, d, interval_d);
			prec_words = nws+1; // full precision to add term in.
			gmpadd(sk1, interval_sk, sk0, interval_sk, sk0, interval_sk, prec_words, d, interval_d);
			...
			(mpr_get(sk1, interval_sk, 1) != 0.0 && 
			mpr_get(sk1, interval_sk, 2) >= mpr_get(sk0, interval_sk, 2) - prec_words));
		...
		if(nq) {
			...
			gmpmul(sk0, interval_sk, sk0, interval_sk, sk1, interval_sk, prec_words, d, interval_d);
			//mpmuld(sk1, 2.0, 0, sk2, prec_words);
			gmpmuld(sk1, interval_sk, 2.0, 0, sk2, interval_sk, prec_words, d, interval_d);
			//mpsub(f, sk2, sk0, prec_words);
			gmpsub(f, 1, sk2, interval_sk, sk0, interval_sk, prec_words, d, interval_d);
			for(int i=1;i<nq;i++) {
				//mpmul(sk0, sk0, sk1, prec_words);
				gmpmul(sk0, interval_sk, sk0, interval_sk, sk1, interval_sk, prec_words, d, interval_d);
				//mpmuld(sk1, 2.0, 0, sk2, prec_words);
				...
				gmpsub(sk2, interval_sk, f, 1, sk0, interval_sk, prec_words, d, interval_d);
			}
		}      
	}
	...
	if(nq) {
		gmpmul(sk0, interval_sk, sk0, interval_sk, sk2, interval_sk, prec_words, d, interval_d);
		gmpsub(f, 1, sk2, interval_sk, sk3, interval_sk, prec_words, d, interval_d);
		gmpsqrt(sk3, interval_sk, sk1, interval_sk, prec_words, d, interval_d);
		if(neg) {
			//sk1[1] = -sk1[1];
			mpr_sign( sk1, interval_sk, 1 );
		}
	} else {
		gmpeq(sk0, interval_sk, sk1, interval_sk, prec_words);
		gmpmul(sk0, interval_sk, sk0, interval_sk, sk2, interval_sk, prec_words, d, interval_d);
		gmpsub(f, 1, sk2, interval_sk, sk3, interval_sk, prec_words, d, interval_d);
		gmpsqrt(sk3, interval_sk, sk0, interval_sk, prec_words, d, interval_d);
	}
	...
	gmpeq(d_pi_over_256_cosine_table + kc, SIN_COS_TABLE_SIZE, sk2, interval_sk, prec_words);
	//mpeq(*pi_over_256_sine_table[kc], sk3, prec_words);
	gmpeq(d_pi_over_256_sine_table + kc, SIN_COS_TABLE_SIZE, sk3, interval_sk, prec_words);
	
	if (kb < 0) {
		//sk3[1] = -sk3[1];
		mpr_sign( sk3, interval_sk, 1 );
	}
	...
	gmpmul(sk0, interval_sk, sk2, interval_sk, sk4, interval_sk, prec_words, d, interval_d);
	//mpmul(sk1, sk3, sk5, prec_words);
	gmpmul(sk1, interval_sk, sk3, interval_sk, sk5, interval_sk, prec_words, d, interval_d);
	//mpsub(sk4, sk5, sk6, prec_words);
	gmpsub(sk4, interval_sk, sk5, interval_sk, sk6, interval_sk, prec_words, d, interval_d);
	//mpmul(sk1, sk2, sk4, prec_words);
	gmpmul(sk1, interval_sk, sk2, interval_sk, sk4, interval_sk, prec_words, d, interval_d);
	//mpmul(sk0, sk3, sk5, prec_words);
	gmpmul(sk0, interval_sk, sk3, interval_sk, sk5, interval_sk, prec_words, d, interval_d);
	//mpadd(sk4, sk5, sk1, prec_words);
	gmpadd(sk4, interval_sk, sk5, interval_sk, sk1, interval_sk, prec_words, d, interval_d);
	//mpeq(sk6, sk0, prec_words);
	gmpeq(sk6, interval_sk, sk0, interval_sk, prec_words);
  ...
  case 0: 
	  ...
	  if( x != NULL ) {
		gmpeq(sk0, interval_sk, x, interval_x, prec_words);
	  }
	  //mpeq(sk1, y, prec_words);
	  if( y != NULL ) {
		gmpeq(sk1, interval_sk, y, interval_y, prec_words);
	  }
	  break;
  case 1:
	  if( x != NULL ) {
		  //mpeq(sk1, x, prec_words);
		  gmpeq(sk1, interval_sk, x, interval_x, prec_words);
		  //x[1] = - x[1];
		  mpr_set(x, interval_x, 1, - mpr_get(x, interval_x, 1));
	  }
	  if( y != NULL ) {
		//mpeq(sk0, y, prec_words);
		gmpeq(sk0, interval_sk, y, interval_y, prec_words);
	  }
	  break;
  case -1:
		if( x != NULL ) {
...
void gmpsin( const double* a, const int interval_a, 
	...
	gmpcssn( a, interval_a, NULL, 0, c, interval_c, prec_words, d, interval_d,
		sin_table, cos_table, 
		sk0, sk1, sk2, sk3, sk4, sk5, sk6, interval_sk );
}
...
void gmpcos( const double* a, const int interval_a, 
			 ...
			 double* sk0, double* sk1, double* sk2, double* sk3, 
			 double* sk4, double* sk5, double* sk6, const int interval_sk ) {
	gmpcssn( a, interval_a, c, interval_c, NULL, 0, prec_words, d, interval_d,
		sin_table, cos_table, 
		sk0, sk1, sk2, sk3, sk4, sk5, sk6, interval_sk );
}
