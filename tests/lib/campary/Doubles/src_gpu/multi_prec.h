...
__host__ __device__ void  certifAddExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
template <int pR, int p1, int p2>
__host__ __device__ void     truncAddExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
template <int pR, int p1>
__host__ __device__ void   truncAddExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
template <int pR, int p1, int p2>
__host__ __device__ void   QD_LikeAddExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
template <int pR, int p1>
__host__ __device__ void QD_LikeAddExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );

//********************* MUL *********************//
template <int pR, int p1, int p2>
__host__ __device__ void    certifMulExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
template <int pR, int p1>
__host__ __device__ void  certifMulExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
template <int pR, int p1, int p2>
__host__ __device__ void     truncMulExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
template <int pR, int p1>
__host__ __device__ void   truncMulExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
template <int pR, int p1, int p2>
__host__ __device__ void   QD_LikeMulExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
template <int pR, int p1>
__host__ __device__ void QD_LikeMulExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );

//********************* DIV *********************//
template <int pR, int p1>
__host__ __device__ void   invExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
template <int pR>
__host__ __device__ void invExpans_d( multi_prec<pR> &res, const double val );
template <int pR, int p1, int p2>
__host__ __device__ void   divExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
template <int pR, int p1>
__host__ __device__ void divExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
template <int pR, int p1>
__host__ __device__ void divExpans_d( multi_prec<pR> &res, const double val, const multi_prec<p1> &mp1 );

//********************* SQRT *********************//
template <int pR, int p1>
__host__ __device__ void      invSqrtExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
template <int pR>
__host__ __device__ void    invSqrtExpans_d( multi_prec<pR> &res, const double val );
template <int pR, int p1>
__host__ __device__ void   sqrtNewtonExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
template <int pR>
__host__ __device__ void sqrtNewtonExpans_d( multi_prec<pR> &res, const double val );
template <int pR, int p1>
...
public:
	...
	__host__ __device__ multi_prec(const double x);
	__host__ __device__ multi_prec(const double *datap, const int precp);
	...
	__host__ __device__ const double* getData() const;
	__host__ __device__ void setData(const double *datap, const int precp);
	__host__ __device__ void setData(const double *datap);
	__host__ __device__ void setElement(const double datap, const int index);
  ...
  __host__ __device__ char* prettyPrintBF(){
	...
	__host__ __device__ multi_prec& operator =(const double other){
		setData(&other, 1);
		return *this;
	}
...
/////////////////////////////////////////////////////////////////////////
  ...
  __host__ __device__ multi_prec<prec>& operator *=(const double val){
	...
	__host__ __device__ friend multi_prec operator +(const double val, const multi_prec<prec> &mp1){
		multi_prec<prec> res(mp1.getData(), prec);
		return res += val;
	}
  ...
  __host__ __device__ friend multi_prec operator /(const double val, const multi_prec<prec> &mp1){
	...
	template <int pR, int p1>
	__host__ __device__ friend void  certifAddExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
	template <int pR, int p1, int p2>
  __host__ __device__ friend void     truncAddExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
  template <int pR, int p1>
  __host__ __device__ friend void   truncAddExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
	template <int pR, int p1, int p2>
	__host__ __device__ friend void   QD_LikeAddExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
	template <int pR, int p1>
	__host__ __device__ friend void QD_LikeAddExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );

	//********************* MUL *********************//
	template <int pR, int p1, int p2>
	__host__ __device__ friend void    certifMulExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
	template <int pR, int p1>
	__host__ __device__ friend void  certifMulExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
  template <int pR, int p1, int p2>
  __host__ __device__ friend void     truncMulExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
  template <int pR, int p1>
  __host__ __device__ friend void   truncMulExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
  template <int pR, int p1, int p2>
	__host__ __device__ friend void   QD_LikeMulExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
  template <int pR, int p1>
	__host__ __device__ friend void QD_LikeMulExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );

	//********************* DIV *********************//
	template <int pR, int p1>
	__host__ __device__ friend void   invExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
	template <int pR>
	__host__ __device__ friend void invExpans_d( multi_prec<pR> &res, const double val );
	template <int pR, int p1, int p2>
	__host__ __device__ friend void   divExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2 );
	template <int pR, int p1>
	__host__ __device__ friend void divExpans_d( multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val );
  template <int pR, int p1>
  __host__ __device__ friend void divExpans_d( multi_prec<pR> &res, const double val, const multi_prec<p1> &mp1 );

	//********************* SQRT *********************//
  template <int pR, int p1>
  __host__ __device__ friend void      invSqrtExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
  template <int pR>
  __host__ __device__ friend void    invSqrtExpans_d( multi_prec<pR> &res, const double val );
	template <int pR, int p1>
	__host__ __device__ friend void   sqrtNewtonExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
	template <int pR>
  __host__ __device__ friend void sqrtNewtonExpans_d( multi_prec<pR> &res, const double val );
	template <int pR, int p1>
  __host__ __device__ friend void    sqrtHeronExpans( multi_prec<pR> &res, const multi_prec<p1> &mp1 );
	template <int pR>
  __host__ __device__ friend void  sqrtHeronExpans_d( multi_prec<pR> &res, const double val );
};

/*definitions*/
//********************* Constructors *********************//
template <int prec>
__host__ __device__ multi_prec<prec>::multi_prec(const double x){
	data[0] = x;
  for(int i=1; i<prec; i++) data[i] = 0;
}
template <int prec>
__host__ __device__ multi_prec<prec>::multi_prec(const double *datap,const int precp){
...
template <int prec>
__host__ __device__ const double* multi_prec<prec>::getData()const{ return data; }
template <int prec>
__host__ __device__ void multi_prec<prec>::setData(const double *datap, const int precp){
  for(int i=0; i<min(prec, precp); i++)	data[i]=datap[i];
  for(int i=min(prec, precp); i<prec; i++) data[i]=0;
}
template <int prec>
__host__ __device__ void multi_prec<prec>::setData(const double *datap){  
  for(int i=0; i<prec; i++)	data[i] = datap[i];
}
template <int prec>
__host__ __device__ void multi_prec<prec>::setElement(const double datap, const int index){
	data[index] = datap;
}
...
/** Computes mp1 + mp2 and puts the result in res (an ulp-nonoverlapping expansion). **/
/** QD generalized addition + renormalize_fast **/
template <int pR, int p1, int p2>
__host__ __device__ void QD_LikeAddExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2){
  const int p = ((p1>=p2)) ? p1 : p2;
	if(p==p1) baileyAdd_fast<p1,p2,pR>(mp1.data, mp2.data, res.data);
  else baileyAdd_fast<p2,p1,pR>(mp2.data, mp1.data, res.data);
}
template <int pR, int p1>
__host__ __device__ void QD_LikeAddExpans_d(multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val){
  renorm2L_4Add1<p1,pR>(mp1.data, val, res.data);
...
/** Computes mp1 * mp2 and puts the result in res (an ulp-nonoverlapping expansion). **/
/** QD generalized multiplication + renormalize_fast **/
template <int pR, int p1, int p2>
__host__ __device__ void QD_LikeMulExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2){
  const int p = ((p1>=p2)) ? p1 : p2;
	if(p==p1) baileyMul_fast<p1,p2,pR>(mp1.data, mp2.data, res.data);
  else baileyMul_fast<p2,p1,pR>(mp2.data, mp1.data, res.data);
}
template <int pR, int p1>
__host__ __device__ void QD_LikeMulExpans_d(multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val){
	baileyMul_fast<p1,1,pR>(mp1.data, &val, res.data);
}
...
template <int pR, int p1>
__host__ __device__ void invExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1){
	invNewtonUp_fast<p1,pR>(mp1.data, res.data);
}
template <int pR>
__host__ __device__ void invExpans_d(multi_prec<pR> &res, const double val){
	invNewtonUp_fast<1,pR>(&val, res.data);
}

template <int pR, int p1, int p2>
__host__ __device__ void divExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1, const multi_prec<p2> &mp2){
  divNewton_fast<p1,p2,pR>(mp1.data, mp2.data, res.data);
}
template <int pR, int p1>
__host__ __device__ void divExpans_d(multi_prec<pR> &res, const multi_prec<p1> &mp1, const double val){
  divNewton_fast<p1,1,pR>(mp1.data, &val, res.data);
}
template <int pR, int p1>
__host__ __device__ void divExpans_d(multi_prec<pR> &res, const double val, const multi_prec<p1> &mp1){
  divNewton_fast<1,p1,pR>(&val, mp1.data, res.data);
}
...
template <int pR, int p1>
__host__ __device__ void invSqrtExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1){
  sqrtInvNewton_fast<p1,pR>(mp1.data, res.data);
}
template <int pR>
__host__ __device__ void invSqrtExpans_d(multi_prec<pR> &res, const double val){
  sqrtInvNewton_fast<1,pR>(&val, res.data);
}

template <int pR, int p1>
__host__ __device__ void sqrtNewtonExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1){
  sqrtNewton_fast<p1,pR>(mp1.data, res.data);
}
template <int pR>
__host__ __device__ void sqrtNewtonExpans_d(multi_prec<pR> &res, const double val){
	sqrtNewton_fast<1,pR>(&val, res.data);
}
...
template <int pR, int p1>
__host__ __device__ void sqrtHeronExpans(multi_prec<pR> &res, const multi_prec<p1> &mp1){
  sqrtHeron_fast<p1,pR>(mp1.data, res.data);
}
template <int pR>
__host__ __device__ void sqrtHeronExpans_d(multi_prec<pR> &res, const double val){
  sqrtHeron_fast<1,pR>(&val, res.data);
}
...
template <int p>
__host__ __device__ bool multi_prec<p>::operator ==(const double &mp2) const{
...
template <int p>
__host__ __device__ bool multi_prec<p>::operator !=(const double &mp2) const{ return !((*this) == mp2); }
...
__host__ __device__ int read(const char *s, multi_prec<pS> &mp) {
  ...
  if (e != 0) r *= pow((float)10.0,e); //(qd_real(10.0) ^ e);
