 ...
 */
template<int prec>
__global__ void campary_spmv_ellpack_kernel(int m, int n, multi_prec<prec> *data, int *indices, multi_prec<prec> *x, multi_prec<prec> *y) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int indice = 0;
    for (int j = 0; j < n; j++) {
        if( i < m ){
            indice = indices[j * m + i];
            //TODO опять же суммирование (нехорошо)
            y[i] = y[i] + data[j * m + i] * x[indice];
        }
        __syncthreads();
    }
    __syncthreads();
 ...
 */
template<int prec>
void campary_spmv_ellpack_test(int m, int n, int maxNonZeros, int lenx, int leny, mp_float_ptr data, int *indices,
    ...
    multi_prec<prec> *hdata = new multi_prec<prec>[m * maxNonZeros];

    //mpfr_data for conversion mp_float -> mpfr -> string_sci
    mpfr_t *mpfrX = new mpfr_t[n];
    mpfr_t *mpfrY = new mpfr_t[m];
    mpfr_t *mpfrData = new mpfr_t[m * maxNonZeros];
    ...
    cudaMalloc(&ddata, sizeof(multi_prec<prec>) * m * maxNonZeros);
    cudaMalloc(&dindices, sizeof(int) * m * maxNonZeros);

    //convert to mpfr
    #pragma omp parallel for
    for(int i = 0; i < n; i ++){
        mpfr_init2(mpfrX[i], MP_PRECISION * 5);
        mp_get_mpfr(mpfrX[i], &x[i]);
    }
    #pragma omp parallel for
    for(int i = 0; i < m; i ++){
        mpfr_init2(mpfrY[i], MP_PRECISION * 5);
        mp_get_mpfr(mpfrY[i], &y[i]);
    }
    #pragma omp parallel for
    for(int i = 0; i < m * maxNonZeros; i ++){
        mpfr_init2(mpfrData[i], MP_PRECISION * 5);
    ...
    cudaMemcpy(ddata, hdata, sizeof(multi_prec<prec>) * m * maxNonZeros, cudaMemcpyHostToDevice);
    ...
    for(int i = 0; i < repeats; i ++){
        ...
        cudaMemcpy(dy, hy, sizeof(multi_prec<prec>) * m, cudaMemcpyHostToDevice);
        StartCudaTimer();
        campary_spmv_ellpack_kernel<prec><<<BLOCKS, CAMPARY_VECTOR_MULTIPLY_THREADS>>>(m, n, ddata, dindices, dx, dy);
        EndCudaTimer();
    }
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
