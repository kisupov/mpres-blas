...
__global__ void campary_mpspmv_jad_kernel(const int m, const int maxnzr, const jad_t jad, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec>
void test_campary_mpspmv_jad(const int m, const int n, const int maxnzr, const int nnz, const jad_t &jad, mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    //Launch
    StartCudaTimer();
    campary_mpspmv_jad_kernel<32, prec><<<blocks, threads, sizeof(multi_prec<prec>) * threads>>>(m, maxnzr, djad, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
