...
#ifndef TEST_MPRES_PCGCSRLS_JACOBI_CUH
#define TEST_MPRES_PCGCSRLS_JACOBI_CUH

#include "../../tsthelper.cuh"
#include "../../logger.cuh"
#include "../../timers.cuh"
#include "sparse/cgcsrls/pcgcsrls_jacobi.cuh"


void test_mpres_pcgcsrls_jacobi(const int n, const int nnz, const csr_t &matrix, const double * rhs, const double tol, const double maxit) {
    InitCudaTimer();
    Logger::printDash();
    PrintTimerName("[GPU] MPRES-BLAS PCG solver (mp_pcgcsrls_jacobi)");
    ...
    cudaMalloc(&dprecond, sizeof(mp_float_t) * n);
    //Initial guess
    ...
    mdiag(matrix, n, temp); //M = main diagonal of A
    #pragma omp parallel for
    for(int i = 0; i < n; i++){
        assert(temp[i] != 0.0);
        mp_set_d(&hx[i], (1.0 / temp[i]));
    }
    delete [] temp;
    cudaMemcpy(dprecond, hx, sizeof(mp_float_t) * n, cudaMemcpyHostToDevice);
    //GPU matrix
    csr_t dmatrix;
    ...
    //Launch
    StartCudaTimer();
    int iters = mp_pcgcsrls_jacobi(n, dmatrix, drhs, dx, dr, dp, dq, dz, dprecond, tol, maxit)
    EndCudaTimer();
    PrintCudaTimer("took");
...
#endif //TEST_MPRES_PCGCSRLS_JACOBI_CUH
