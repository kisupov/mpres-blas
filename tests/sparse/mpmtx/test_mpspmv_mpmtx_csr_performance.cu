...
void test(const char * MATRIX_PATH, const int M, const int N, const int LINES, const int NNZ, const bool SYMM, const string DATATYPE) {
    ...
    Logger::printStars();
    test_mpres_mpspmv_mpmtx_csr_vector<2>(M, N, NNZ, IRP, JA, AS, vectorX);
    test_mpres_mpspmv_mpmtx_csr_vector<4>(M, N, NNZ, IRP, JA, AS, vectorX);
    test_mpres_mpspmv_mpmtx_csr_vector<8>(M, N, NNZ, IRP, JA, AS, vectorX);
    test_mpres_mpspmv_mpmtx_csr_vector<16>(M, N, NNZ, IRP, JA, AS, vectorX);
    test_mpres_mpspmv_mpmtx_csr_vector<32>(M, N, NNZ, IRP, JA, AS, vectorX);
    test_mpres_mpspmv_mpmtx_csr_scalar(M, N, NNZ, IRP, JA, AS, vectorX);
    test_mpres_mpspmv_mpmtx_csr_2stage(M, N, NNZ, IRP, JA, AS, vectorX);
    Logger::printStars();
    test_campary_mpspmv_mpmtx_csr_vector<CAMPARY_PRECISION, 2>(M, N, NNZ, IRP, JA, AS, vectorX, INP_DIGITS);
    test_campary_mpspmv_mpmtx_csr_vector<CAMPARY_PRECISION, 4>(M, N, NNZ, IRP, JA, AS, vectorX, INP_DIGITS);
    test_campary_mpspmv_mpmtx_csr_vector<CAMPARY_PRECISION, 8>(M, N, NNZ, IRP, JA, AS, vectorX, INP_DIGITS);
    test_campary_mpspmv_mpmtx_csr_vector<CAMPARY_PRECISION, 16>(M, N, NNZ, IRP, JA, AS, vectorX, INP_DIGITS);
