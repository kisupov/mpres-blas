/*
 *  Performance test for the MPRES-BLAS library SpMV routine mpspmv_mpmtx_jad (multiple precision matrix)
 *
...
#ifndef TEST_MPRES_MPSPMV_MPMTX_JAD_CUH
#define TEST_MPRES_MPSPMV_MPMTX_JAD_CUH
...
/////////
void test_mpres_mpspmv_mpmtx_jad(const int m, const int n, const int maxnzr, const int nnz, const int *ja, const int *jcp,
                                 const double *as, const int *perm_rows, const mpfr_t *x) {
    InitCudaTimer();
    Logger::printDash();
    PrintTimerName("[GPU] MPRES-BLAS mpspmv_mpmtx_jad");
    ...
    //Launch
    StartCudaTimer();
    cuda::mpspmv_mpmtx_jad<<<blocks, threads>>>(m, maxnzr, das, dja, djcp, dperm_rows, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_MPRES_MPSPMV_MPMTX_JAD_CUH