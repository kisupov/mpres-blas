/*
 *  Performance test for the CAMPARY library SpMV routine campary_spmv_mpmtx_jad_kernel (multiple precision matrix)
...
#ifndef TEST_CAMPARY_SPMV_MPMTX_JAD_CUH
#define TEST_CAMPARY_SPMV_MPMTX_JAD_CUH
 ...
 */
template<int prec>
__global__ void campary_spmv_mpmtx_jad_kernel(const int m, const int maxnzr, const int *ja, const multi_prec<prec> *as, const int *jcp, const int *perm_rows, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec>
void test_campary_spmv_mpmtx_jad(const int m, const int n, const int maxnzr, const int nnz, const int *ja, const int *jcp, const double *as, const int *perm_rows, mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    //Launch
    StartCudaTimer();
    campary_spmv_mpmtx_jad_kernel<prec><<<blocks, threads>>>(m, maxnzr, dja, das, djcp, dperm_rows, dx, dy);
    EndCudaTimer();
...
#endif //TEST_CAMPARY_SPMV_MPMTX_JAD_CUH
