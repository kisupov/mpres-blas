/*
 *  Performance test for the MPRES-BLAS library SpMV routine mp_spmv_mpmtx_jad (multiple precision matrix)
 *
...
#ifndef TEST_MPRES_SPMV_MPMTX_JAD_CUH
#define TEST_MPRES_SPMV_MPMTX_JAD_CUH
...
/////////
void test_mpres_spmv_mpmtx_jad(const int m, const int n, const int maxnzr, const int nnz, const int *ja, const int *jcp,
                               const double *as, const int *perm_rows, const mpfr_t *x) {
    InitCudaTimer();
    Logger::printDash();
    PrintTimerName("[GPU] MPRES-BLAS mp_spmv_mpmtx_jad");
    ...
    //Launch
    StartCudaTimer();
    cuda::mp_spmv_mpmtx_jad<<<blocks, threads>>>(m, maxnzr, das, dja, djcp, dperm_rows, dx, dy);
    EndCudaTimer();
...
#endif //TEST_MPRES_SPMV_MPMTX_JAD_CUH