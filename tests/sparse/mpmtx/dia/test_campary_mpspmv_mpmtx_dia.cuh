/*
 *  Performance test for the CAMPARY library SpMV routine campary_mpspmv_dia_kernel (multiple precision matrix)
...
#ifndef TEST_CAMPARY_MPSPMV_MPMTX_DIA_CUH
#define TEST_CAMPARY_MPSPMV_MPMTX_DIA_CUH
 ...
 */
template<int prec>
__global__ void campary_mpspmv_dia_kernel(const int m, const int n, const int ndiag, const int *offset, const multi_prec<prec> *as, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec>
void test_campary_mpspmv_mpmtx_dia(const int m, const int n, const int ndiag, const int *offset, const double *as, mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    //Launch
    StartCudaTimer();
    campary_mpspmv_dia_kernel<prec><<<blocks, threads>>>(m, n, ndiag, doffset, das, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_CAMPARY_MPSPMV_MPMTX_DIA_CUH
