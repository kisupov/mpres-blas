/*
 *  Performance test for the CAMPARY library SpMV routine campary_spmv_mpmtx_ell_kernel (multiple precision matrix)
...
#ifndef TEST_CAMPARY_SPMV_MPMTX_ELL_CUH
#define TEST_CAMPARY_SPMV_MPMTX_ELL_CUH
 ...
 */
template<int prec>
__global__ void campary_spmv_mpmtx_ell_kernel(const int m, const int maxnzr, const int *ja, const multi_prec<prec> *as, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec>
void test_campary_spmv_mpmtx_ell(const int m, const int n, const int maxnzr, const int *ja, const double *as, mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    cudaMalloc(&das, sizeof(multi_prec<prec>) * m * maxnzr);
    ...
    cudaMemcpy(das, has, sizeof(multi_prec<prec>) * m * maxnzr, cudaMemcpyHostToDevice);
    ...
    //Launch
    StartCudaTimer();
    campary_spmv_mpmtx_ell_kernel<prec><<<blocks, threads>>>(m, maxnzr, dja, das, dx, dy);
    EndCudaTimer();
...
#endif //TEST_CAMPARY_SPMV_MPMTX_ELL_CUH
