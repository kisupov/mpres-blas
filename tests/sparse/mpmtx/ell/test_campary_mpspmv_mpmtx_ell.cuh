...
#ifndef TEST_CAMPARY_MPSPMV_MPMTX_ELLPACK_CUH
#define TEST_CAMPARY_MPSPMV_MPMTX_ELLPACK_CUH
 ...
 */
template<int prec>
__global__ void campary_mpspmv_ellpack_kernel(const int m, const int maxnzr, const int *ja, const multi_prec<prec> *as, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec>
void test_campary_mpspmv_mpmtx_ell(const int m, const int n, const int maxnzr, const int *ja, const double *as, mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    //Launch
    StartCudaTimer();
    campary_mpspmv_ellpack_kernel<prec><<<blocks, threads>>>(m, maxnzr, dja, das, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_CAMPARY_MPSPMV_MPMTX_ELLPACK_CUH
