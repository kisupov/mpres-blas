/*
 *  Performance test for the MPRES-BLAS library SpMV routine mp_spmv_mpmtx_ell (multiple precision matrix)
 *
...
#ifndef TEST_MPRES_SPMV_MPMTX_ELL_CUH
#define TEST_MPRES_SPMV_MPMTX_ELL_CUH
...
/////////
void test_mpres_spmv_mpmtx_ell(const int m, const int n, const int maxnzr, const int *ja, const double *as, const mpfr_t *x){
    InitCudaTimer();
    Logger::printDash();
    PrintTimerName("[GPU] MPRES-BLAS mp_spmv_mpmtx_ell");
    ...
    //Launch
    StartCudaTimer();
    cuda::mp_spmv_mpmtx_ell<<<blocks, threads>>>(m, maxnzr, dja, das, dx, dy);
    EndCudaTimer();
...
#endif //TEST_MPRES_SPMV_MPMTX_ELL_CUH