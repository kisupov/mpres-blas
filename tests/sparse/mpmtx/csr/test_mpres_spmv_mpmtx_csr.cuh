/*
 *  Performance test for the MPRES-BLAS library SpMV routine mp_spmv_mpmtx_csr (multiple precision matrix)
 *
...
#ifndef TEST_MPRES_SPMV_MPMTX_CSR_CUH
#define TEST_MPRES_SPMV_MPMTX_CSR_CUH
...
/////////
void test_mpres_spmv_mpmtx_csr(const int m, const int n, const int nnz, const csr_t &csr, const mpfr_t * x){
    InitCudaTimer();
    Logger::printDash();
    PrintTimerName("[GPU] MPRES-BLAS mp_spmv_mpmtx_csr");
    ...
    //Launch
    StartCudaTimer();
    cuda::mp_spmv_mpmtx_csr<<<blocks, threads>>>(m, dirp, dja, das, dx, dy);
    EndCudaTimer();
...
#endif //TEST_MPRES_SPMV_MPMTX_CSR_CUH