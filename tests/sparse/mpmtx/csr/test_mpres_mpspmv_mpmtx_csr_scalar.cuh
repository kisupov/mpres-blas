/*
 *  Performance test for the MPRES-BLAS library SpMV routine mpspmv_mpmtx_csr_scalar (multiple precision matrix)
 *
...
#ifndef TEST_MPRES_MPSPMV_MPMTX_CSR_SCALAR_CUH
#define TEST_MPRES_MPSPMV_MPMTX_CSR_SCALAR_CUH
...
void test_mpres_mpspmv_mpmtx_csr_scalar(const int m, const int n, const int nnz, const int *irp, const int *ja, const double *as, const mpfr_t * x){
    ...
    //Launch
    StartCudaTimer();
    cuda::mpspmv_mpmtx_csr_scalar<<<blocks, threads>>>(m, dirp, dja, das, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_MPRES_MPSPMV_MPMTX_CSR_SCALAR_CUH