/*
 *  Performance test for the CAMPARY library SpMV routine campary_spmv_mpmtx_csrv_kernel (multiple precision matrix)
...
#ifndef TEST_CAMPARY_SPMV_MPMTX_CSRV_CUH
#define TEST_CAMPARY_SPMV_MPMTX_CSRV_CUH
...
__global__ void campary_spmv_mpmtx_csrv_kernel(const int m, const int *irp, const int *ja, const multi_prec<prec> *as, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec, int threadsPerRow>
void test_campary_spmv_mpmtx_csrv(const int m, const int n, const int nnz, const csr_t &csr, mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    //Launch
    StartCudaTimer();
    campary_spmv_mpmtx_csrv_kernel<prec, threadsPerRow><<<blocks, threads, sizeof(multi_prec<prec>) * threads>>>(m, dirp, dja, das, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_CAMPARY_SPMV_MPMTX_CSRV_CUH
