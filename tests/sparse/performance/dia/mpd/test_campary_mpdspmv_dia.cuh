/*
 *  Performance test for the CAMPARY library SpMV routine campary_mpspmv_dia_kernel (double precision matrix)
...
#ifndef TEST_CAMPARY_MPDSPMV_DIA_CUH
#define TEST_CAMPARY_MPDSPMV_DIA_CUH
...
template<int prec>
void test_campary_mpdspmv_dia(const int m, const int n, const int ndiag, const int *offset, const double *as,  mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    //Launch
    StartCudaTimer();
    campary_mpdspmv_dia_kernel<32, prec><<<blocks, threads>>>(m, n, ndiag, doffset, das, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_CAMPARY_MPDSPMV_DIA_CUH
