/*
 *  Performance test for the CUMP library SpMV routine cump_mpspmv_dia_kernel (multiple precision matrix)
...
#ifndef TEST_CUMP_MPSPMV_DIA_CUH
#define TEST_CUMP_MPSPMV_DIA_CUH
 ...
 */
__global__ void cump_mpspmv_dia_kernel(const int m, const int n, const int ndiag, const int *offset, mpf_array_t as, mpf_array_t x, mpf_array_t y, mpf_array_t buf) {
...
void test_cump_mpspmv_dia(const int m, const int n, const int ndiag, const int *offset, const double *as, mpfr_t *x, const int prec, const int convert_digits){
    ...
    //Launch
    StartCudaTimer();
    cump_mpspmv_dia_kernel<<<blocks, threads>>>(m, n, ndiag, doffset, das, dx, dy, dbuf);
    EndCudaTimer();
    PrintCudaTimer("took");
...
#endif //TEST_CUMP_MPSPMV_DIA_CUH
