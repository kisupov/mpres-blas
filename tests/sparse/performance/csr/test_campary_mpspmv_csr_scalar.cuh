/*
 *  Performance test for the CAMPARY library SpMV routine campary_mpspmv_csr_scalar_kernel (multiple precision matrix)
...
#ifndef TEST_CAMPARY_MPSPMV_CSR_SCALAR_CUH
#define TEST_CAMPARY_MPSPMV_CSR_SCALAR_CUH
 ...
 */
template<int prec>
__global__ void campary_mpspmv_csr_scalar_kernel(const int m, const int *irp, const int *ja, const multi_prec<prec> *as, const multi_prec<prec> *x, multi_prec<prec> *y) {
...
template<int prec>
void test_campary_mpspmv_csr_scalar(const int m, const int n, const int nnz, const int *irp, const int *ja, const double *as,  mpfr_t *x, const int convert_prec) {
    Logger::printDash();
    InitCudaTimer();
    ...
    printf("\tMatrix size (MB): %lf\n", double(sizeof(multi_prec<prec>)) * nnz /  double(1024 * 1024));
    ...
    //Launch
    StartCudaTimer();
    campary_mpspmv_csr_scalar_kernel<prec><<<blocks, threads>>>(m, dirp, dja, das, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
...
#endif //TEST_CAMPARY_MPSPMV_CSR_SCALAR_CUH
