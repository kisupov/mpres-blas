/*
 *  Performance test for the CUMP library SpMV routine cump_mpspmv_csr_scalar_kernel (multiple precision matrix)
...
#ifndef TEST_CUMP_MPSPMV_CSR_SCALAR_CUH
#define TEST_CUMP_MPSPMV_CSR_SCALAR_CUH
 ...
 */
__global__ void cump_mpspmv_csr_scalar_kernel(const int m, const int *irp,  const int *ja, mpf_array_t as, mpf_array_t x, mpf_array_t y, mpf_array_t buf) {
...
void test_cump_mpspmv_csr_scalar(const int m, const int n, const int nnz, const int *irp, const int *ja, const double *as,  mpfr_t *x, const int prec, const int convert_digits){
    Logger::printDash();
    InitCudaTimer();
    ...
    //Memory requirements
    double sizeOfMatrix = print_cump_csr_memory_consumption(m, nnz, prec);
    ...
    //Launch
    StartCudaTimer();
    cump_mpspmv_csr_scalar_kernel<<<blocks, threads>>>(m, dirp, dja, das, dx, dy, dbuf);
    EndCudaTimer();
    PrintCudaTimer("took");
...
#endif //TEST_CUMP_MPSPMV_CSR_SCALAR_CUH
