...
void test(const char * MATRIX_PATH, const int M, const int N, const int LINES, const int MAXNZ, const int NNZ, const bool SYMM, const string DATATYPE) {
    ...
    test_mpres_mpdspmv_jad(M, N, MAXNZ, NNZ, JA, JCP, AS, PERM_ROWS, vectorX);
    test_campary_mpdspmv_jad<CAMPARY_PRECISION>(M, N, MAXNZ, NNZ, JA, JCP, AS, PERM_ROWS, vectorX, INP_DIGITS);
