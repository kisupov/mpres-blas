...
//  SpMV jad kernel
/////////
void test_mpres_mpdspmv_jad(const int m, const int n, const int maxnz, const int nnz, const int *ja, const int *jcp,
    ...
    //Launch
    StartCudaTimer();
    cuda::mpdspmv_jad<<<blocks, threads>>>(m, maxnz, das, dja, djcp, dperm_rows, dx, dy);
    EndCudaTimer();
    PrintCudaTimer("took");
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
