...
void test(const char * MATRIX_PATH, const int M, const int N, const int LINES, const int MAXNZR, const bool SYMM, const string DATATYPE) {
    ...
    test_mpres_mpspmv_ellpack_scalar(M, N, MAXNZR, JA, AS, vectorX);
    test_mpres_mpspmv_ellpack_2stage(M, N, MAXNZR, JA, AS, vectorX);
    test_campary_mpspmv_ellpack<CAMPARY_PRECISION>(M, N, MAXNZR, JA, AS, vectorX, INP_DIGITS);
