...
#include "test_mpres_axpy.cuh"
#include "test_mpfr_axpy.cuh"
#include "test_openblas_axpy.cuh"
...
void test() {
    ...
    //Launch tests
    test_openblas(N, alpha[0], vectorX, vectorY, REPEAT_TEST);
    test_mpfr(N, alpha[0], vectorX, vectorY, REPEAT_TEST);
    test_mpres(N, alpha[0], vectorX, vectorY, REPEAT_TEST);
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
    //Cleanup
    for (int i = 0; i < N; i++) {
