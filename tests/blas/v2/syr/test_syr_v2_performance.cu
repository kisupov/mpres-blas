/*
 *  Performance test for BLAS SYR routines (symmetric rank-1 update)
 *
...
#include "test_double_syr.cuh"
#include "test_mpfr_syr.cuh"
#include "test_openblas_syr.cuh"
#include "test_cublas_syr.cuh"
#include "test_mpres_syr.cuh"
#include "test_campary_syr.cuh"
...
void test() {
    ...
    //Launch tests
    test_openblas(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, REPEAT_TEST);
    test_double(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, REPEAT_TEST);
    test_mpfr(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, REPEAT_TEST);
    test_cublas(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, REPEAT_TEST);
    test_double_cuda(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, REPEAT_TEST);
    test_mpres_syr(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, REPEAT_TEST);
    test_campary_syr<CAMPARY_PRECISION>(UPLO, N, alpha[0], vectorX, INCX, matrixA, LDA, INP_DIGITS, REPEAT_TEST);
    checkDeviceHasErrors(cudaDeviceSynchronize());
    cudaCheckErrors();
    //Cleanup
...
int main() {
    initialize();
    Logger::beginTestDescription(Logger::BLAS_SYR_PERFORMANCE_TEST);
